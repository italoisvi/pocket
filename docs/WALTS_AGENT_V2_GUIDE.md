...
while (iteration < maxIterations) {
  ...
  if (toolsExecutedThisRound > 0) {
    ...
    messages.push({
      role: 'system',
      content: 'Agora responda ao usuário com base nos dados obtidos. NÃO chame mais ferramentas.'
    });
  }
...
ÚLTIMOS GASTOS:
${userContext.recentExpenses.slice(0, 5).map(e => 
  `- ${e.establishment_name}: R$ ${e.amount} (${e.category})`
).join('\n')}

ORÇAMENTOS:
${userContext.budgets.map(b => 
  `- ${b.category_id}: R$ ${b.amount}/${b.period_type}`
).join('\n')}
...
if (toolsExecuted > 0) {
  conversationMessages.push({
    role: 'system',
    content: 'IMPORTANTE: Responda ao usuário agora. Não chame mais ferramentas.'
  });
}
```
